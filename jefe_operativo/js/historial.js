...
let ultimosResultados = null;  // Guardar últimos resultados para filtrar

// =====================================================
// INICIALIZACIÓN
// =====================================================
document.addEventListener('DOMContentLoaded', async () => {
    console.log('🚀 Inicializando página de historial (Jefe Operativo)...');
...
function aplicarFiltros() {
    // Si no hay resultados guardados, no hacer nada
...
async function buscarHistorial() {
    ...
    try {
        let url = `${API_URL}/jefe-operativo/historial-vehiculo?placa=${encodeURIComponent(placa)}`;
...
function limpiarBusqueda() {
    ...
    // Recargar últimas órdenes
    cargarUltimasOrdenes();
}

// =====================================================
// RENDERIZADO DE ÓRDENES (función genérica)
// =====================================================
...
async function cargarUltimasOrdenes() {
    ...
    try {
        const response = await fetch(`${API_URL}/jefe-operativo/ultimas-ordenes?limite=10`, {
            headers: getHeaders()
        });
...
async function verDetalleCompletoOrden(idOrden) {
    try {
        ...
        const response = await fetch(`${API_URL}/jefe-operativo/detalle-completo-orden/${idOrden}`, {
            headers: getHeaders()
        });
...
function mostrarModalDetalleCompleto(detalle) {
    ...
    const recepcionHtml = `
        <div class="detalle-seccion recepcion">
            ...
            <div class="detalle-grid">
                <div class="detalle-item">
                    <span class="detalle-label">Jefe Operativo que registró</span>
            ...
            <div class="detalle-descripcion" style="margin-top: 0.75rem;">
    ...
    if (detalle.diagnosticos_tecnicos && detalle.diagnosticos_tecnicos.length > 0) {
        diagnosticosTecnicosHtml = detalle.diagnosticos_tecnicos.map(dt => `
            <div class="detalle-seccion diagnostico-tecnico">
                ...
                ` : ''}
                ${dt.observaciones ? `
                    <div class="detalle-descripcion" style="margin-top: 0.75rem; background: rgba(193,18,31,0.1);">
