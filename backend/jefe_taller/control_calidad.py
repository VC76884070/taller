...
def obtener_historial_asignaciones(id_orden):
    """Obtener historial de asignaciones de una orden"""
    try:
        asignaciones = supabase.table('asignaciontecnico') \
            .select('id_tecnico, tipo_asignacion, fecha_hora_inicio, fecha_hora_final, usuario!inner(id, nombre)') \
            .eq('id_orden_trabajo', id_orden) \
        ...
        for a in (asignaciones.data or []):
            ...
            resultado.append({
                'tecnico': nombre,
                'tipo': a.get('tipo_asignacion'),
                'fecha_inicio': a.get('fecha_hora_inicio'),
                'fecha_final': a.get('fecha_hora_final')
            })
        return resultado
    except Exception as e:
...
def obtener_ordenes_pendientes(current_user):
    ...
    Estados: VehiculoArmado, ReparacionCompletada
    """
    try:
        estado_filtro = request.args.get('estado', 'all')
        
        # Estados pendientes de revisión
        ...
        for orden in result.data:
            ...
            # Obtener fecha de finalización (de la última asignación completada)
            ...
            # Obtener fecha de inicio de la reparación (CORREGIDO)
            ...
            ordenes.append({
                ...
                'cliente_contacto': usuario.get('contacto', ''),
...
def obtener_ordenes_finalizadas(current_user):
    ...
    try:
        ...
        print(f"📊 Órdenes finalizadas encontradas: {len(result.data) if result.data else 0}")
        ...
        for orden in result.data:
            ...
            # Obtener fecha de finalización (usar fecha_ingreso como fallback)
            fecha_finalizacion = orden.get('fecha_ingreso')
            
            # Intentar obtener fecha de avance de control de calidad
            try:
                ...
                logger.warning(f"No se pudo obtener avance: {e}")
            
            # Obtener comentarios de aprobación
            comentarios = None
            try:
                ...
                logger.warning(f"No se pudo obtener comentarios: {e}")
        ...
        print(f"✅ {len(ordenes)} órdenes formateadas")
        return jsonify({'success': True, 'ordenes': ordenes}), 200
        
    except Exception as e:
        logger.error(f"Error obteniendo órdenes finalizadas: {str(e)}")
        import traceback
        traceback.print_exc()
        return jsonify({'error': str(e)}), 500
...
def obtener_detalle_orden(current_user, id_orden):
    ...
    try:
        ...
        historial_tecnicos = obtener_historial_asignaciones(id_orden)
        ...
        return jsonify({
            ...
            'detalle': {
                ...
                'tecnicos_nombres': tecnicos_nombres,
                'historial_tecnicos': historial_tecnicos,
                'recepcion': {
...
def finalizar_orden(current_user, id_orden):
    """Aprobar y finalizar una orden (cambiar a estado Finalizado)"""
    try:
        ...
        # Registrar avance (sin usar tabla que no existe)
        ...
        logger.error(f"Error finalizando orden: {str(e)}")
        return jsonify({'error': str(e)}), 500
...
def rechazar_orden(current_user, id_orden):
    ...
    try:
        ...
        # Solo se pueden rechazar órdenes en estados pendientes de revisión
        if estado_actual not in ['VehiculoArmado', 'ReparacionCompletada']:
        ...
        for t in (tecnicos.data or []):
            ...
            # Crear nueva asignación de reparación (SIN id_jefe_taller)
            supabase.table('asignaciontecnico').insert({
                'id_orden_trabajo': id_orden,
                'id_tecnico': t['id_tecnico'],
                'tipo_asignacion': 'reparacion',
                'fecha_hora_inicio': ahora
                # ELIMINADO: 'id_jefe_taller': current_user['id']
            }).execute()
        ...
        supabase.table('instrucciones_tecnico_historial').insert({
            ...
            'instrucciones': f"[REVISIÓN NECESARIA]\n\n{instrucciones}",
            'fecha_envio': ahora,
            'leida': False
        }).execute()
        
        # Registrar en historial (usar tabla que existe)
        try:
            supabase.table('seguimientoorden').insert({
                'id_orden_trabajo': id_orden,
                'estado': 'EnReparacion',
                'motivo_pausa': f"Trabajo rechazado - {instrucciones[:200]}",
                'fecha_hora_cambio': ahora,
                'notificaciones_enviadas': 1
            }).execute()
        except Exception as e:
            logger.warning(f"No se pudo registrar seguimiento: {e}")
        ...
        logger.error(f"Error rechazando orden: {str(e)}")
        import traceback
        traceback.print_exc()
        return jsonify({'error': str(e)}), 500
