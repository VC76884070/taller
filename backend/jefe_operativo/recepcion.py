...
def actualizar_actividad_sesion(codigo):
    """Actualiza la marca de tiempo de actividad de una sesión"""
    try:
        supabase.table('sesion_colaborativa') \
...
def crear_orden_desde_sesion(datos, current_user, sesion_codigo=None, reintentos=3):
    ...
    for intento in range(reintentos):
        try:
            ...
            # Buscar segundo jefe operativo en la sesión
...
def obtener_detalle_completo_orden(id_orden):
    """
    Obtiene todos los detalles de una orden para el reporte
    """
    try:
        # Obtener orden
...
def marcar_editando(current_user):
    """Marca que un usuario está editando una sección"""
    try:
        data = request.get_json()
        codigo_sesion = data.get('codigo')
        seccion = data.get('seccion')
...
def liberar_edicion(current_user):
    """Libera una sección que estaba siendo editada"""
    try:
        data = request.get_json()
        codigo_sesion = data.get('codigo')
        seccion = data.get('seccion')
    ...
    try:
        ...
        reporte_data = {
            ...
            'orden': {
                ...
                'estado_global': detalle['estado_global']
            },
            'cliente': {
                'nombre': detalle['cliente_nombre'],
                'telefono': detalle['cliente_telefono'],
                'ubicacion': detalle['cliente_ubicacion'],
            ...
            'fotos': detalle['fotos'],
            'descripcion': {
                'texto': detalle['transcripcion_problema'],
                'audio_url': detalle['audio_url']
            },
            'jefe_operativo': detalle['jefe_operativo'],
            'jefe_operativo_2': detalle['jefe_operativo_2']
        }
